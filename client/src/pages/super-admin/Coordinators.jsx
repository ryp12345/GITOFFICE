...
export default function SuperAdminCoordinatorsPage() {
  ...
  const load = async () => {
    ...
    try {
      const res = await getCoordinators(token);
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      if (editingId) {
        await updateCoordinator(editingId, payload, token);
        showNotification('Coordinator updated successfully!', 'success');
      } else {
        await createCoordinator(payload, token);
  ...
  const remove = async (id) => {
    if (!window.confirm('Delete this coordinator? This action is permanent.')) return;
    try {
  ...
  const filtered = useMemo(() => {
    ...
    return sorted.filter((r) => {
      ...
      const type = String(r.employee_type || '').toLowerCase();
        ...
        <main className="flex-1 overflow-auto p-6">
          <div className="max-w-7xl mx-auto">
            ...
            <div className="mb-12 text-center">
              <h1 className="mb-2 text-4xl font-extrabold text-gray-900">Coordinators</h1>
              <p className="text-lg text-gray-600">Create, update and manage coordinator definitions</p>
            </div>
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-blue-600">
                    <tr>
                      ...
                      <th className="px-6 py-4 text-left text-xs font-medium text-white uppercase tracking-wider">Coordinator</th>
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {filtered.length === 0 ? (
                      <tr>
                        <td colSpan="4" className="px-6 py-12 text-center text-gray-500">No coordinators found</td>
                      </tr>
                    ) : (
                      paginated.map((row, idx) => (
                        <tr key={row.id} className={`${idx % 2 === 0 ? 'bg-white' : 'bg-gray-50'} hover:bg-blue-50 transition-colors duration-150`}>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{row.employee_type || '-'}</td>
                  ...
                  <div className="inline-block overflow-hidden text-left align-bottom transition-all transform bg-white rounded-lg shadow-xl sm:my-8 sm:align-middle sm:max-w-2xl sm:w-full">
                    <div className="px-6 py-4 bg-blue-600">
                      <div className="flex items-center justify-between">
                        <h3 className="text-lg font-medium leading-6 text-white">{editingId ? 'Edit Coordinator' : 'Add Coordinator'}</h3>
                      ...
                      <form className="space-y-5" onSubmit={submit}>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                          <div className="md:col-span-2">
                            <label className="block mb-2 text-sm font-medium text-gray-700">Coordinator Name *</label>
                            <input
                              type="text"
                              value={form.name}
                          ...
                          <div className="md:col-span-2">
                            <label className="block mb-2 text-sm font-medium text-gray-700">Employee Type *</label>
                            <select
                        ...
                        <div className="flex justify-end space-x-4 pt-4">
                          <button
                            type="button"
                            onClick={onClose}
                            className="inline-flex justify-center px-6 py-3 text-sm font-medium text-gray-700 bg-white border border-gray-300 rounded-lg shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"
                          >
                            Cancel
                          </button>
                          <button
                            type="submit"
                            className="inline-flex justify-center px-6 py-3 text-sm font-medium text-white border border-transparent rounded-lg shadow-sm bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500"
                          >
