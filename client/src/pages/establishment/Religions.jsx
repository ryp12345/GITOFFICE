...
export default function ReligionsPage() {
  ...
  const load = async () => {
    ...
    setLoading(true);
    try {
      const res = await getReligions(token);
      const data = res?.data?.data || [];
      setRows(Array.isArray(data) ? data : []);
    } catch {
      setRows([]);
      showNotification('Failed to load religions', 'error');
    }
    setLoading(false);
  };
  ...
  const submit = async (e) => {
    ...
    const payload = {
      religion_name: form.religion_name.trim(),
      status: form.status,
    };

    try {
      if (editingId) {
        await updateReligion(editingId, payload, token);
        showNotification('Religion updated successfully!', 'success');
      } else {
        await createReligion(payload, token);
        showNotification('Religion created successfully!', 'success');
      }
  ...
  const remove = async (id) => {
    if (!window.confirm('Delete this religion? This action is permanent.')) return;

    try {
      await deleteReligion(id, token);
  ...
  const filtered = useMemo(() => {
    ...
    return sorted.filter((row) => (
      row.religion_name?.toLowerCase().includes(q) ||
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-blue-600">
                    <tr>
                      ...
                      <th className="px-6 py-4 text-left text-xs font-medium text-white uppercase tracking-wider">Religion Name</th>
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {loading ? (
                      ...
                      <tr><td colSpan="4" className="px-6 py-12 text-center text-gray-500">No religions found</td></tr>
                    ) : (
                      paginated.map((row, idx) => (
                        <tr key={row.id} className={`${idx % 2 === 0 ? 'bg-white' : 'bg-gray-50'} hover:bg-blue-50 transition-colors duration-150`}>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{row.religion_name}</td>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                            <div className="flex items-center justify-center space-x-2">
                              <button
                                ...
                                title="View Religion Castes"
                                onClick={() => navigate(`/caste-categories?religionId=${row.id}`)}
                              >
                  ...
                  <div className="inline-block overflow-hidden text-left align-bottom transition-all transform bg-white rounded-lg shadow-xl sm:my-8 sm:align-middle sm:max-w-2xl sm:w-full">
                    <div className="px-6 py-4 bg-blue-600">
                      <div className="flex items-center justify-between">
                        <h3 className="text-lg font-medium leading-6 text-white">{editingId ? 'Edit Religion' : 'Add Religion'}</h3>
                      ...
                      <form className="space-y-5" onSubmit={submit}>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                          <div className="md:col-span-2">
                            <label className="block mb-2 text-sm font-medium text-gray-700">Religion Name *</label>
                            <input
                              type="text"
                              value={form.religion_name}
                              onChange={(e) => setForm({ ...form, religion_name: e.target.value })}
                        ...
                        <div className="flex justify-end gap-3 pt-2">
                          <button
                            type="button"
                            onClick={onClose}
                            className="px-6 py-2.5 font-medium text-gray-700 bg-gray-100 rounded-lg hover:bg-gray-200"
                          >
                            Cancel
                          </button>
                          <button
                            type="submit"
                            className="px-6 py-2.5 font-medium text-white bg-blue-600 rounded-lg hover:bg-blue-700"
                          >
