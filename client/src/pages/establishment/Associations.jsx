...
export default function AssociationsPage() {
  ...
  const load = async () => {
    ...
    try {
      const res = await getAssociations(token);
      const data = res?.data?.data || [];
      // normalize backend field names (asso_name -> association_name)
      const normalized = (Array.isArray(data) ? data : []).map((r) => ({
        ...r,
        association_name: r.association_name || r.asso_name || r.assoName || '',
      }));
      setRows(normalized);
    } catch (e) {
      setRows([]);
  ...
  const openEdit = (row) => {
    setEditingId(row.id);
    setForm({ associationName: row.association_name || '', category: row.category || 'Associated', status: row.status || 'active' });
    setIsModalOpen(true);
  };
  ...
  const submit = async (e) => {
    ...
    try {
      const payload = {
        asso_name: form.associationName,
        category: form.category,
        status: form.status,
      };

      if (editingId) {
        await updateAssociation(editingId, payload, token);
        showNotification('Association updated successfully!', 'success');
      } else {
        await createAssociation(payload, token);
  ...
  const remove = async (id) => {
    if (!confirm('Delete this association? This action is permanent.')) return;
    try {
      await deleteAssociation(id, token);
      load();
          ...
          <div className="max-w-7xl mx-auto">
            ...
            <div className="flex flex-col items-start justify-between gap-4 mb-6 sm:flex-row sm:items-center">
              <div className="relative w-full sm:w-72">
                <input value={search} onChange={(e)=>setSearch(e.target.value)} placeholder="Search associations..." className="w-full py-2 pl-10 pr-4 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" />
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-blue-600">
                    <tr>
                      ...
                      <th className="px-6 py-4 text-left text-xs font-medium text-white uppercase tracking-wider">Association Name</th>
                    ...
                    {filtered.length === 0 ? (
                      ...
                      paginated.map((row, idx) => (
                        <tr key={row.id} className={`${idx % 2 === 0 ? 'bg-white' : 'bg-gray-50'} hover:bg-blue-50 transition-colors duration-150`}>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{row.association_name}</td>
                    ...
                    <div className="px-6 py-5 bg-white">
                      ...
                      <form className="space-y-5" onSubmit={submit}>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                          ...
                          <div className="md:col-span-1">
                            ...
                            >
                              ...
                              <option value="Temporary associated">Temporary associated</option>
                        ...
                        <div className="flex justify-end space-x-4 pt-4">
                          ...
                          <button type="submit" className="inline-flex justify-center px-6 py-3 text-sm font-medium text-white border border-transparent rounded-lg shadow-sm bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">{editingId ? 'Update Association' : 'Create Association'}</button>
                        </div>
                      </form>
                    </div>
                  </div>
