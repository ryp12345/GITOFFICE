...
export default function InstitutionsPage() {
  ...
  const load = async () => {
    ...
    setLoading(true);
    try {
      const res = await getInstitutions(token);
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      if (editingId) {
        await updateInstitution(editingId, payload, token);
        showNotification('Institution updated successfully!', 'success');
      } else {
        await createInstitution(payload, token);
  ...
  const remove = async (id) => {
    if (!window.confirm('Delete this institution? This action is permanent.')) return;
        ...
        <main className="flex-1 overflow-auto p-6">
          <div className="max-w-7xl mx-auto">
            ...
            <div className="mb-12 text-center">
              <h1 className="mb-2 text-4xl font-extrabold text-gray-900">Institutions</h1>
              <p className="text-lg text-gray-600">Create, update and manage institutions</p>
            </div>
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-blue-600">
                    <tr>
                      ...
                      <th className="px-6 py-4 text-left text-xs font-medium text-white uppercase tracking-wider">Institution Name</th>
                      <th className="px-6 py-4 text-left text-xs font-medium text-white uppercase tracking-wider">Acronym</th>
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {loading ? (
                      ...
                      <tr><td colSpan="5" className="px-6 py-12 text-center text-gray-500">No institutions found</td></tr>
                    ) : (
                      paginated.map((row, idx) => (
                        <tr key={row.id} className={`${idx % 2 === 0 ? 'bg-white' : 'bg-gray-50'} hover:bg-blue-50 transition-colors duration-150`}>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{row.acronym}</td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{row.created_at ? new Date(row.created_at).toLocaleDateString() : '-'}</td>
                  ...
                  <div className="inline-block overflow-hidden text-left align-bottom transition-all transform bg-white rounded-lg shadow-xl sm:my-8 sm:align-middle sm:max-w-2xl sm:w-full">
                    <div className="px-6 py-4 bg-blue-600">
                      <div className="flex items-center justify-between">
                        <h3 className="text-lg font-medium leading-6 text-white">{editingId ? 'Edit Institution' : 'Add Institution'}</h3>
                    ...
                    <div className="px-6 py-5 bg-white">
                      ...
                      <form className="space-y-5" onSubmit={submit}>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                          ...
                          <div className="md:col-span-2">
                            <label className="block mb-2 text-sm font-medium text-gray-700">Acronym *</label>
                            <input
                              type="text"
                              value={form.acronym}
                        ...
                        <div className="flex justify-end space-x-4 pt-4">
                          ...
                          <button type="submit" className="inline-flex justify-center px-6 py-3 text-sm font-medium text-white border border-transparent rounded-lg shadow-sm bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">{editingId ? 'Update Institution' : 'Create Institution'}</button>
                        </div>
                      </form>
                    </div>
                  </div>
