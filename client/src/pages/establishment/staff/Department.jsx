...
export default function Department({ staff, setNotification, onDepartmentUpdated }) {
  ...
  const openCreateModal = () => {
    setEditingRow(null);
    setError('');
    setForm({ department_id: '', start_date: toInputDate(new Date()), end_date: '', reason: '', gcr: '' });
    setShowModal(true);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (!form.department_id || !form.start_date) { setError('Department and start date are required'); return; }
    setSaving(true);
    try {
      const payload = {
      ...
      if (editingRow?.id) {
        if (editingRow.status === 'closed' && form.status === 'active') {
          await updateStaffDepartment(staff.id, editingRow.id, { status: 'active' });
        } else {
          await updateStaffDepartment(staff.id, editingRow.id, payload);
        }
        if (typeof setNotification === 'function') setNotification({ show: true, message: 'Department updated successfully', type: 'success' });
      } else {
        await createStaffDepartment(staff.id, payload);
        if (typeof setNotification === 'function') setNotification({ show: true, message: 'Department added successfully', type: 'success' });
      }
  ...
  const handleDelete = async (row) => {
    ...
    const duration = calcDuration(row.start_date, row.end_date);
    const isWithinOneMonth = duration.includes('0 years') && parseInt(duration) <= 1;
    const hasDesignationReference = staff?.designations?.some(
      (d) => d.pivot?.dept_id === row.department_id && d.pivot?.status === 'active'
    );
    if (row.status !== 'active' || !isWithinOneMonth || hasDesignationReference) {
      setError('Delete not allowed. Department must be active, within 1 month duration, and not referenced by any active designation.');
      return;
    }
    if (!window.confirm('Are you sure you want to delete this department record?')) return;
    try {
  ...
  return (
    <div className="w-full space-y-6">
      <div className="flex items-center justify-between gap-4">
        ...
        <button onClick={openCreateModal} className="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 text-sm font-semibold">Change Department</button>
      </div>
        ...
        <table className="min-w-full">
          ...
          <tbody>
            {departmentRows.map((d, idx) => {
              ...
              const duration = calcDuration(d.start_date, d.end_date);
              const isWithinOneMonth = duration.includes('0 years') && parseInt(duration) <= 1;
              const hasDesignationRef = staff?.designations?.some((des) => des.pivot?.dept_id === d.department_id && des.pivot?.status === 'active');
              const canDelete = d.status === 'active' && isWithinOneMonth && !hasDesignationRef;
              return (
                <tr key={idx} className={isClosed ? 'bg-gray-200' : 'even:bg-gray-50'}>
                  ...
                  <td className="px-3 py-2 border-b text-sm">
                    <div className="flex items-center justify-center space-x-2">
                      <button onClick={() => openEditModal(d)} className="p-2 text-white transition-colors duration-200 bg-blue-600 rounded-lg hover:bg-blue-700" title={isClosed ? 'Activate' : 'Edit'}>
                      ...
                      <button onClick={() => handleDelete(d)} className="p-2 text-white transition-colors duration-200 bg-red-600 rounded-lg hover:bg-red-700 disabled:opacity-50 disabled:cursor-not-allowed" title="Delete" disabled={!canDelete}>
          ...
          <div className="w-full max-w-2xl rounded-xl bg-white shadow-2xl">
            <div className="flex items-center justify-between border-b border-gray-200 px-6 py-4">
              <h3 className="text-lg font-bold text-gray-800">
                {editingRow?.status === 'closed' ? 'Activate Department' : editingRow ? 'Edit Department' : 'Change Department'}
              </h3>
              <button onClick={closeModal} className="text-gray-500 hover:text-gray-700 text-xl font-bold">&times;</button>
            </div>
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              {editingRow?.status === 'closed' ? (
                <div className="flex items-center gap-2">
                  <input type="radio" name="status" value="active" checked={form.status === 'active'} onChange={(e) => setForm((p) => ({ ...p, status: e.target.value }))} className="ti-form-radio" />
                ...
                <>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="md:col-span-2">
                      ...
                      <select className="w-full rounded-lg border border-gray-300 px-3 py-2" value={form.department_id} onChange={(e) => setForm((prev) => ({ ...prev, department_id: e.target.value }))} required>
                        <option value="">Select department</option>
                        {departmentOptions.map((opt) => <option key={opt.id} value={opt.id}>{opt.dept_name}</option>)}
                      </select>
                    </div>
                    <div>
                      <label className="block text-sm font-semibold text-gray-700 mb-1">Effect from Date</label>
                      <input type="date" className="w-full rounded-lg border border-gray-300 px-3 py-2" value={form.start_date} onChange={(e) => setForm((prev) => ({ ...prev, start_date: e.target.value }))} required />
                    </div>
                    <div>
                      <label className="block text-sm font-semibold text-gray-700 mb-1">End Date</label>
                      <input type="date" className="w-full rounded-lg border border-gray-300 px-3 py-2" value={form.end_date} onChange={(e) => setForm((prev) => ({ ...prev, end_date: e.target.value }))} />
                    </div>
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <label className="block text-sm font-semibold text-gray-700 mb-1">Reason</label>
                      <input type="text" className="w-full rounded-lg border border-gray-300 px-3 py-2" value={form.reason} onChange={(e) => setForm((prev) => ({ ...prev, reason: e.target.value }))} placeholder="Reason" />
                    </div>
                    <div>
                      <label className="block text-sm font-semibold text-gray-700 mb-1">GC Resolution</label>
                      <input type="text" className="w-full rounded-lg border border-gray-300 px-3 py-2" value={form.gcr} onChange={(e) => setForm((prev) => ({ ...prev, gcr: e.target.value }))} placeholder="GCR" />
                    </div>
                  </div>
                </>
              )}
              ...
              <div className="flex justify-end gap-3 pt-2">
                ...
                <button type="submit" className="px-4 py-2 rounded-md bg-blue-600 text-white hover:bg-blue-700" disabled={saving}>
                  {saving ? 'Saving...' : editingRow?.status === 'closed' ? 'Activate' : editingRow ? 'Update' : 'Save'}
                </button>
              </div>
            </form>
          </div>
