...
export default function TaxRegime({ staff, setNotification, onTaxRegimeUpdated }) {
  ...
  const handleDelete = async (row) => {
    ...
    if (!window.confirm('Delete this tax regime record?')) return;

    try {
      await deleteStaffTaxRegime(staff.id, row.id);
      if (typeof setNotification === 'function') {
        setNotification({ show: true, message: 'Tax regime deleted successfully', type: 'success' });
      }
      await fetchRows();
      if (typeof onTaxRegimeUpdated === 'function') await onTaxRegimeUpdated();
    } catch (err) {
      setError(err?.response?.data?.message || 'Failed to delete tax regime');
    }
  };
