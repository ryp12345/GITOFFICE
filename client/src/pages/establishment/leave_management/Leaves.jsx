// ...existing code from d:/GITOFFICE/client/src/pages/establishment/Leaves.jsx...
...
export default function LeavesPage() {
  ...
  const openEdit = (row) => {
    ...
    setForm({
      ...
      max_entitlement: row.max_entitlement || '',
  ...
  const submit = async (e) => {
    ...
    if (!form.min_days) {
      setError('Min days is required');
      return;
    }
    if (!form.vacation_type) {
      setError('Vacation type is required');
      return;
    }
    if (!form.applicable_to) {
      setError('Applicable to is required');
      return;
    }
    if (!form.leave_wef) {
      setError('Leave WEF is required');
      return;
    }
    try {
      const payload = { ...form };
      if (editingId) {
        ...
        await axios.post('/leaves', payload, { headers: { Authorization: `Bearer ${token}` } });
        showNotification('Leave created successfully!', 'success');
      }
      onClose();
      load();
    } catch (e) {
  ...
  const remove = async (id) => {
    if (!window.confirm('Delete this leave? This action is permanent.')) return;
    try {
      await axios.delete(`/leaves/${id}`, { headers: { Authorization: `Bearer ${token}` } });
      load();
        ...
        <main className="flex-1 overflow-auto p-6">
          <div className="max-w-7xl mx-auto">
            ...
            <div className="mb-12 text-center">
              <h1 className="mb-2 text-4xl font-extrabold text-gray-900">Leaves</h1>
              <p className="text-lg text-gray-600">Create, update and manage leaves</p>
            </div>
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-blue-600">
                    <tr>
                      ...
                      <th className="px-6 py-4 text-left text-xs font-medium text-white uppercase tracking-wider">Long Name</th>
                      <th className="px-6 py-4 text-left text-xs font-medium text-white uppercase tracking-wider">Short Name</th>
                      <th className="px-6 py-4 text-left text-xs font-medium text-white uppercase tracking-wider">Max Entitlement</th>
                      <th className="px-6 py-4 text-left text-xs font-medium text-white uppercase tracking-wider">Min Days</th>
                      <th className="px-6 py-4 text-left text-xs font-medium text-white uppercase tracking-wider">Max Days</th>
                      <th className="px-6 py-4 text-left text-xs font-medium text-white uppercase tracking-wider">Vacation Type</th>
                      <th className="px-6 py-4 text-left text-xs font-medium text-white uppercase tracking-wider">Applicable To</th>
                      <th className="px-6 py-4 text-left text-xs font-medium text-white uppercase tracking-wider">Leave WEF</th>
                      <th className="px-6 py-4 text-left text-xs font-medium text-white uppercase tracking-wider">Leave End Date</th>
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {loading ? (
                      ...
                      <tr><td colSpan="12" className="px-6 py-12 text-center text-gray-500">No leaves found</td></tr>
                    ) : (
                      paginated.map((row, idx) => (
                        <tr key={row.id} className={`${idx % 2 === 0 ? 'bg-white' : 'bg-gray-50'} hover:bg-blue-50 transition-colors duration-150`}>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{row.longname}</td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{row.shortname}</td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{row.max_entitlement}</td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{row.min_days}</td>
                  ...
                  <div className="inline-block overflow-hidden text-left align-bottom transition-all transform bg-white rounded-lg shadow-xl sm:my-8 sm:align-middle sm:max-w-2xl sm:w-full">
                    <div className="px-6 py-4 bg-blue-600">
                      <div className="flex items-center justify-between">
                        <h3 className="text-lg font-medium leading-6 text-white">{editingId ? 'Edit Leave' : 'Add Leave'}</h3>
                      ...
                      <form className="space-y-5" onSubmit={submit}>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                          <div className={editingId ? "md:col-span-1" : "md:col-span-2"}>
                            <label className="block mb-2 text-sm font-medium text-gray-700">Long Name *</label>
                            <input
                              type="text"
                              value={form.longname}
                          ...
                          <div className="md:col-span-1">
                            <label className="block mb-2 text-sm font-medium text-gray-700">Max Entitlement</label>
                            <input
                              type="number"
                          ...
                          <div className="md:col-span-1">
                            <label className="block mb-2 text-sm font-medium text-gray-700">Min Days *</label>
                            <input
                              type="number"
                              value={form.min_days}
                          ...
                          <div className="md:col-span-1">
                            <label className="block mb-2 text-sm font-medium text-gray-700">Max Days</label>
                            <input
                              type="number"
                              value={form.max_days}
                          ...
                          <div className="md:col-span-1">
                            <label className="block mb-2 text-sm font-medium text-gray-700">Leave WEF *</label>
                            <input
                              type="date"
                          ...
                          <div className="md:col-span-1">
                            <label className="block mb-2 text-sm font-medium text-gray-700">Leave End Date</label>
                            <input
                              type="date"
                        ...
                        <div className="flex justify-end space-x-4 pt-4">
                          ...
                          <button type="submit" className="inline-flex justify-center px-6 py-3 text-sm font-medium text-white border border-transparent rounded-lg shadow-sm bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">{editingId ? 'Update Leave' : 'Create Leave'}</button>
                        </div>
                      </form>
                    </div>
                  </div>
