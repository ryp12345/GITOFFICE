...
export default function QualificationsPage() {
  ...
  const openEdit = (row) => {
    ...
    setForm({
      ...
      qual_shortname: row.qual_shortname || '',
  ...
  const submit = async (e) => {
    ...
    if (!form.qual_name.trim()) {
      setError('Qualification name is required');
      return;
    }
    if (!form.qual_shortname.trim()) {
      setError('Qualification short name is required');
      return;
    }
    ...
    const payload = {
      qual_name: form.qual_name.trim(),
      qual_shortname: form.qual_shortname.trim(),
      status: form.status,
    };
    ...
    try {
      if (editingId) {
        ...
        showNotification('Qualification created successfully!', 'success');
      }
  ...
  const remove = async (id) => {
    if (!window.confirm('Delete this qualification? This action is permanent.')) return;
  ...
  const filtered = useMemo(() => {
    ...
    return sorted.filter((row) => (
      row.qual_name?.toLowerCase().includes(q) ||
      row.qual_shortname?.toLowerCase().includes(q) ||
        ...
        <main className="flex-1 overflow-auto p-6">
          <div className="max-w-7xl mx-auto">
            ...
            <div className="mb-12 text-center">
              <h1 className="mb-2 text-4xl font-extrabold text-gray-900">Qualifications</h1>
              <p className="text-lg text-gray-600">Create, update and manage qualifications</p>
            </div>
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-blue-600">
                    <tr>
                      ...
                      <th className="px-6 py-4 text-left text-xs font-medium text-white uppercase tracking-wider">Qualification Name</th>
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {loading ? (
                      ...
                      <tr><td colSpan="5" className="px-6 py-12 text-center text-gray-500">No qualifications found</td></tr>
                    ) : (
                      paginated.map((row, idx) => (
                        <tr key={row.id} className={`${idx % 2 === 0 ? 'bg-white' : 'bg-gray-50'} hover:bg-blue-50 transition-colors duration-150`}>
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{row.qual_name}</td>
                          <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{row.qual_shortname}</td>
                      ...
                      <form className="space-y-5" onSubmit={submit}>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                          <div className="md:col-span-2">
                            ...
                            <input
                              type="text"
                              value={form.qual_name}
                              onChange={(e) => setForm({ ...form, qual_name: e.target.value })}
                          ...
                          <div>
                            ...
                            <input
                              type="text"
                              value={form.qual_shortname}
                              onChange={(e) => setForm({ ...form, qual_shortname: e.target.value })}
                              className="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
