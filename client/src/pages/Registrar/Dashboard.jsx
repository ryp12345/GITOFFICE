...
export default function RegistrarDashboard() {
    ...
    async function fetchStats() {
      try {
        ...
        const teachingCount = staffArr.filter((s) => {
          ...
          return type === 'teaching' || type === 'teachingstaff' || type === 'teacher';
        }).length;
          ...
          const rows = combined.map((d) => {
            ...
            if (Array.isArray(staffList) && staffList.length > 0) {
              const matched = staffList.find(
                (s) =>
                  String(s.EmployeeCode || s.employeecode || s.employeecode) === String(code) ||
                  String(s.employeecode || s.EmployeeCode || s.employeecode) === String(code)
              );
              if (matched) {
                ...
                base.EmployeeName =
                  base.EmployeeName ||
                  `${matched.fname || ''} ${matched.mname || ''} ${matched.lname || ''}`.trim() ||
                  matched.full_name ||
                  matched.name ||
                  base.EmployeeName;
              }
  ...
  const filteredAttendance = useMemo(() => {
    ...
    return attendance.filter(
      (r) =>
        (r.EmployeeName || r.full_name || r.employeeName || r.EmployeeCode || '')
          .toString()
          .toLowerCase()
          .includes(q) ||
      ...
      <div className="flex flex-1 min-h-0">
        ...
        <main className="flex-1 overflow-auto p-6">
          <div className="min-h-full rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
            <h2 className="text-2xl font-semibold text-slate-900">Registrar Dashboard</h2>
            <p className="mt-2 text-slate-600">Welcome to the Registrar panel.</p>
                  ...
                  <div className="bg-white rounded-lg max-w-2xl w-full p-4">
                    <div className="flex justify-between items-center mb-4">
                      <h4 className="font-semibold">
                        Log Details - <span className="text-blue-600 font-semibold">{selectedPunches.employee}</span>
                      </h4>
