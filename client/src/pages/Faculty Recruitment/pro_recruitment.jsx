...
import {
  createProfessorApplication,
  deleteProfessorApplication,
  exportProfessorApplications,
...
function computeAssociateProfessorYears(fromDate, toDate) {
  if (!fromDate) return '0.00';
  const from = new Date(fromDate);
  const to = toDate ? new Date(toDate) : new Date();
  from.setHours(0, 0, 0, 0);
  to.setHours(0, 0, 0, 0);
  if (Number.isNaN(from.getTime()) || Number.isNaN(to.getTime()) || to <= from) return '0.00';
...
function renderExperienceRemarks(row) {
  const expYrs = toNumber(row.experience_years, 0);
  const short = Math.max(0, 10 - expYrs);

  if (row.experience_remarks === 'Satisfied') {
    return (
      <>
        {`Satisfied (${formatCount(expYrs)} yrs)`}
        {renderStatusIcon(true, 'Requirement Met')}
      </>
    );
  }
  ...
  if (row.experience_remarks === 'Not Satisfied') {
    return (
      <>
        {`Not Satisfied (${formatCount(expYrs)} yrs) - Need ${formatCount(short)} more yrs`}
        {renderStatusIcon(false, 'Requirement Not Met')}
      </>
    );
  }
...
function renderAssociateExperienceRemarks(row) {
  const assocExp = toNumber(row.post_asso_prof_experience, 0);
  const short = Math.max(0, 3 - assocExp);

  if (row.asso_prof_experience_remarks === 'Satisfied') {
    return (
      <>
        {`Satisfied (${formatCount(assocExp)} yrs)`}
        {renderStatusIcon(true, 'Requirement Met')}
      </>
    );
  }

  if (row.asso_prof_experience_remarks === 'Not Satisfied') {
    return (
      <>
        {`Not Satisfied (${formatCount(assocExp)} yrs) - Need ${formatCount(short)} more yrs`}
        {renderStatusIcon(false, 'Requirement Not Met')}
      </>
    );
  }

  return row.asso_prof_experience_remarks || '';
}

function renderResearchRemarks(row) {
  const sci = toNumber(row.papers_in_sci, 0);
  const ugc = toNumber(row.papers_in_ugc, 0);
  const aicte = toNumber(row.papers_in_aicte, 0);
  const total = toNumber(row.research_papers_count, 0);
  const scholars = toNumber(row.research_scholars_count, 0);
  const pathA = total >= 6 && scholars >= 2;
  const pathB = total >= 10;

  if (row.research_remarks === 'Satisfied') {
    return (
      <>
        {`Satisfied - Total: ${formatCount(total)} (SCI: ${formatCount(sci)}, UGC: ${formatCount(ugc)}, AICTE: ${formatCount(aicte)}), Ph.D. Guided: ${formatCount(scholars)}`}
        {pathB ? ' (>=10 publications path)' : pathA ? ' (>=6 publications + >=2 Ph.D. guided path)' : ''}
        {renderStatusIcon(true, 'Requirement Met')}
      </>
    );
  }
  ...
  if (row.research_remarks === 'Not Satisfied') {
    return (
      <>
        {`Not Satisfied - Total: ${formatCount(total)} (SCI: ${formatCount(sci)}, UGC: ${formatCount(ugc)}, AICTE: ${formatCount(aicte)}), Ph.D. Guided: ${formatCount(scholars)} - Need (>=6 pubs and >=2 guided) OR >=10 pubs`}
        {renderStatusIcon(false, 'Requirement Not Met')}
      </>
    );
  }
...
export default function ProRecruitmentPage() {
  ...
  useEffect(() => {
    setForm((prev) => ({
      ...prev,
      post_asso_prof_experience: computeAssociateProfessorYears(prev.from_date_asso_prof, prev.to_date_asso_prof)
    }));
  }, [form.from_date_asso_prof, form.to_date_asso_prof]);
  ...
  const openEdit = (row) => {
    ...
    setForm({
      ...
      industry_designation: row.industry_designation || '',
      associate_level: row.associate_level || '',
      ...
      from_date_asso_prof: row.from_date_asso_prof ? String(row.from_date_asso_prof).slice(0, 10) : '',
      to_date_asso_prof: row.to_date_asso_prof ? String(row.to_date_asso_prof).slice(0, 10) : '',
      post_asso_prof_experience: row.post_asso_prof_experience ?? '',
      papers_in_SCI: row.papers_in_sci ?? '',
      papers_in_UGC: row.papers_in_ugc ?? '',
      papers_in_AICTE: row.papers_in_aicte ?? '',
      research_scholars_count: row.research_scholars_count ?? '',
  ...
  const handleChange = (event) => {
    ...
    setForm((prev) => {
      ...
      if (name === 'type') {
        if (value === 'Education') {
          next.industry_designation = '';
          next.associate_level = '';
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (editingId) {
        await updateProfessorApplication(editingId, payload);
        setNotification({ show: true, message: 'Professor Application updated successfully', type: 'success' });
      } else {
        await createProfessorApplication(payload);
        setNotification({ show: true, message: 'New Professor Application Added successfully', type: 'success' });
      }
  ...
  const handleDelete = async (id) => {
    ...
    try {
      await deleteProfessorApplication(id);
  ...
  const handleExport = async () => {
    try {
      const response = await exportProfessorApplications();
      const blob = new Blob([response.data], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
      const url = URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = url;
      link.download = 'professor_applications.xlsx';
      link.click();
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  <thead className="bg-blue-600">
                    <tr>
                      ...
                      <th className="px-3 py-4 text-left text-xs font-medium text-white uppercase tracking-wider">Advertisement Instance</th>
                      ...
                      <th className="px-3 py-4 text-left text-xs font-medium text-white uppercase tracking-wider">Type</th>
                      <th className="px-3 py-4 text-left text-xs font-medium text-white uppercase tracking-wider">Designation / Industry Designation</th>
                      <th className="px-3 py-4 text-left text-xs font-medium text-white uppercase tracking-wider">Comparison to Associate</th>
                      ...
                      <th className="px-3 py-4 text-left text-xs font-medium text-white uppercase tracking-wider">From Date AssoProf</th>
                      <th className="px-3 py-4 text-left text-xs font-medium text-white uppercase tracking-wider">To Date AssoProf</th>
                      ...
                      <th className="px-3 py-4 text-left text-xs font-medium text-white uppercase tracking-wider">Post AssoProf Exp</th>
                      <th className="px-3 py-4 text-left text-xs font-medium text-white uppercase tracking-wider">Asso Prof Exp Remarks</th>
                      ...
                      <th className="px-3 py-4 text-left text-xs font-medium text-white uppercase tracking-wider">No of PhDs Guided</th>
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {loading ? (
                      <tr><td colSpan="35" className="px-6 py-12 text-center text-gray-500">Loading...</td></tr>
                    ) : filtered.length === 0 ? (
                      <tr><td colSpan="35" className="px-6 py-12 text-center text-gray-500">No applications found</td></tr>
                    ) : (
                      paginated.map((row, idx) => (
                        <tr key={row.id} className={`${idx % 2 === 0 ? 'bg-white' : 'bg-gray-50'} hover:bg-blue-50 transition-colors duration-150`}>
                          ...
                          <td className="px-3 py-2">{row.type === 'Industry' ? row.industry_designation : row.designation}</td>
                          <td className="px-3 py-2">{row.associate_level}</td>
                          ...
                          <td className="px-3 py-2">{formatDateDMY(row.from_date_asso_prof)}</td>
                          <td className="px-3 py-2">{formatDateDMY(row.to_date_asso_prof)}</td>
                          ...
                          <td className="px-3 py-2">{row.post_asso_prof_experience}</td>
                          <td className="px-3 py-2">{renderAssociateExperienceRemarks(row)}</td>
                          ...
                          <td className="px-3 py-2">{row.research_scholars_count}</td>
            ...
            <div className="inline-block overflow-hidden text-left align-bottom transition-all transform bg-white rounded-lg shadow-xl sm:my-8 sm:align-middle sm:max-w-5xl sm:w-full">
              <div className="px-6 py-4 bg-blue-600">
                <div className="flex items-center justify-between">
                  <h3 className="text-lg font-medium leading-6 text-white">{editingId ? 'Edit Professor Application' : 'Add Professor Application'}</h3>
              ...
              <div className="px-6 py-5 bg-white max-h-[75vh] overflow-auto">
                <form className="space-y-5" onSubmit={handleSubmit}>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    <div><label className="block mb-2 text-sm font-medium text-gray-700">PhD University *</label><input name="phd_university" value={form.phd_university} onChange={handleChange} className="block w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" required /></div>
                    ...
                    <div><label className="block mb-2 text-sm font-medium text-gray-700">Type *</label><select name="type" value={form.type} onChange={handleChange} className="block w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" required><option value="">Select</option><option value="Education">Education</option><option value="Industry">Industry</option></select></div>
                    {form.type === 'Education' && <div><label className="block mb-2 text-sm font-medium text-gray-700">Designation</label><select name="designation" value={form.designation} onChange={handleChange} className="block w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"><option value="">Select</option><option value="Assistant Professor">Assistant Professor</option><option value="Associate Professor">Associate Professor</option><option value="Professor">Professor</option></select></div>}
                    {form.type === 'Industry' && <div><label className="block mb-2 text-sm font-medium text-gray-700">Industry Designation</label><input name="industry_designation" value={form.industry_designation} onChange={handleChange} className="block w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" /></div>}
                    {form.type === 'Industry' && <div><label className="block mb-2 text-sm font-medium text-gray-700">Comparison to Associate</label><select name="associate_level" value={form.associate_level} onChange={handleChange} className="block w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"><option value="">Select</option><option value="Lower to Associate">Lower to Associate</option><option value="Greater to Associate">Greater to Associate</option><option value="Equal to Associate">Equal to Associate</option></select></div>}
                    <div><label className="block mb-2 text-sm font-medium text-gray-700">UG Class Obtained *</label><select name="ug_class" value={form.ug_class} onChange={handleChange} className="block w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" required><option value="">Select</option><option value="F">First</option><option value="S">Second</option><option value="T">Third</option></select></div>
                    <div><label className="block mb-2 text-sm font-medium text-gray-700">UG Branches *</label><select name="ug_branches" value={form.ug_branches} onChange={handleChange} className="block w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" required><option value="">Select</option>{ugBranchOptions.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></div>
                    <div><label className="block mb-2 text-sm font-medium text-gray-700">PG Class Obtained *</label><select name="pg_class" value={form.pg_class} onChange={handleChange} className="block w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" required><option value="">Select</option><option value="F">First</option><option value="S">Second</option><option value="T">Third</option></select></div>
                    ...
                    <div><label className="block mb-2 text-sm font-medium text-gray-700">From Date Asso Prof *</label><input type="date" name="from_date_asso_prof" value={form.from_date_asso_prof} onChange={handleChange} className="block w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" required /></div>
                    <div><label className="block mb-2 text-sm font-medium text-gray-700">To Date Asso Prof</label><input type="date" name="to_date_asso_prof" value={form.to_date_asso_prof} onChange={handleChange} className="block w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" /></div>
                    <div><label className="block mb-2 text-sm font-medium text-gray-700">Post Asso Prof Experience (Years)</label><input name="post_asso_prof_experience" value={form.post_asso_prof_experience} className="block w-full px-4 py-3 border border-gray-300 rounded-lg bg-slate-100" readOnly /></div>
                    <div className="md:col-span-2"><h4 className="text-center font-medium text-gray-700">Research papers during cadre of Associate Professor</h4></div>
                    ...
                    <div><label className="block mb-2 text-sm font-medium text-gray-700">No of PhDs Guided as Supervisor</label><input type="number" name="research_scholars_count" value={form.research_scholars_count} onChange={handleChange} className="block w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" /></div>
                    ...
                    <div className="md:col-span-2"><label className="block mb-2 text-sm font-medium text-gray-700">Remarks</label><textarea name="remarks" value={form.remarks} onChange={handleChange} className="block w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" rows="2" /></div>
                  </div>
