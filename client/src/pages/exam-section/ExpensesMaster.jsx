...
export default function ExpensesMasterPage() {
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      if (editingId) {
        await updateExpenseMaster(editingId, payload);
        showNotification('Fastrack Expense Master Updated successfully', 'success');
      } else {
        await createExpenseMaster(payload);
        showNotification('New Fastrack Expense Master Added successfully', 'success');
      }
      onClose();
      load();
    } catch (e) {
  ...
  const remove = async (id) => {
    if (!window.confirm('Are you sure you want to delete this expense master?')) return;
    try {
      await deleteExpenseMaster(id);
      load();
        ...
        <main className="flex-1 overflow-auto p-6">
          <div className="max-w-7xl mx-auto">
            ...
            <div className="mb-12 text-center">
              <h1 className="mb-2 text-4xl font-extrabold text-gray-900">Fastrack Expenses Master Details</h1>
              <p className="text-lg text-gray-600">Create, update and manage expense masters</p>
            </div>

            <div className="flex flex-col items-start justify-between gap-4 mb-6 sm:flex-row sm:items-center">
              <div className="relative w-full sm:w-72">
                <input value={search} onChange={(e)=>setSearch(e.target.value)} placeholder="Search expense masters..." className="w-full py-2 pl-10 pr-4 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" />
                ...
                <table className="min-w-full divide-y divide-gray-200">
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {loading ? (
                      ...
                      <tr><td colSpan="8" className="px-6 py-12 text-center text-gray-500">No expense masters found</td></tr>
                    ) : (
                  ...
                  <div className="inline-block overflow-hidden text-left align-bottom transition-all transform bg-white rounded-lg shadow-xl sm:my-8 sm:align-middle sm:max-w-2xl sm:w-full">
                    <div className="px-6 py-4 bg-blue-600">
                      <div className="flex items-center justify-between">
                        <h3 className="text-lg font-medium leading-6 text-white">{editingId ? 'Edit Fastrack Expenses Master' : 'Add New Fastrack Expenses Master'}</h3>
